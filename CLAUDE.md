...
structured-workflow/               # structured-workflow 插件根目录
  ...
  skills/                          # 7 个 Skills（workflow-init, task-exec, plan-adjust, phase-review, task-auto, workflow-abort, workflow-archive）
  scripts/                         # Python 辅助脚本（init_project.py, archive_workflow.py, abort_workflow.py, setup_autoexec.py）
  references/                      # 参考文档（methodology.md, task-format.md, exception-handling.md, planner-prompts.md, analyzer-prompts.md, handover-template.md）
...
所有脚本通过 `uv run` 在项目根目录执行：

```bash
# 初始化工作流目录结构
uv run structured-workflow/scripts/init_project.py

# 归档已完成的工作流
uv run structured-workflow/scripts/archive_workflow.py

# 中止当前工作流
uv run structured-workflow/scripts/abort_workflow.py

# 配置自动执行环境
uv run structured-workflow/scripts/setup_autoexec.py
```
