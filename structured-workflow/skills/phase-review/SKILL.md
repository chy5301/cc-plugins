...
**置信度过滤**：每项发现先标置信度（0-100），只有置信度 ≥80 的发现才进入严重度分类：
- 🔴 阻断：必须修正才能继续
- 🟡 需修正：应当修正但不阻塞流程
- 🔵 建议：记录供后续参考
...
1. **IDENTIFY**：阶段级验证需要什么证据？（构建通过、测试通过、无新增警告）
2. **RUN**：运行 `workflow.json` 中配置的构建命令和测试命令
3. **READ**：读取完整输出，检查隐藏的警告或跳过的测试
4. **VERIFY**：输出是否匹配预期？
5. **CLAIM**：全部通过才算构建验证通过
...
| 禁令 | 常见借口 | 为什么无效 |
|---|---|---|
| 不放低置信度就报告 | "宁可多报不漏报" | 噪音导致用户审阅疲劳，连 🔴 都会被忽略 |
| 不做范围外修正 | "顺手修了吧反正发现了" | 修正必须可追溯到审计发现，范围外修正应走 plan-adjust |
| 不跳过构建验证 | "代码没改过应该没问题" | 多任务累积效应可能引入隐藏问题 |
