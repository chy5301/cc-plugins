...
- 每次只执行一个任务——多任务并行会导致状态文件冲突和交接记录混乱
- 只做任务范围内的变更——范围外的修改（重构、格式化、顺手修 bug）应记录为新任务
- 避免"如前所述""参见上文"等模糊引用——每个任务的描述需自包含，因为下一个会话没有当前上下文
- 不要跳过验证步骤——未验证的任务可能将错误传递到后续任务
- 状态更新（第 5 步）在完成汇报（第 6 步）之前执行——这保证即使会话中断，进度也不会丢失
- TASK_STATUS.md 的更新是每个任务的必要收尾动作
