...
| 禁令 | 常见借口 | 为什么无效 |
|---|---|---|
| 不做范围外变更 | "顺手改了更好" | 无人值守下范围外修改无人审查，风险更高 |
| 不跳过验证 | "改动太小" | 验证门函数就几条命令，auto 模式更不能省 |
| 不盲目重试失败的修复 | "再试一次应该可以" | 轻量调试失败就标记阻塞，不要反复尝试 |

### 阶段检查点（步骤 7）

| 禁令 | 常见借口 | 为什么无效 |
|---|---|---|
| 不放低置信度就报告 | "宁可多报不漏报" | auto 模式下噪音直接变成无意义的修正动作 |
| 不在审计修正中做额外改动 | "反正都要改不如一起" | 修正必须可追溯到审计发现 |
