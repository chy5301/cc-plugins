...
structured-workflow/               # structured-workflow 插件根目录
  ...
  commands/                        # 斜杠命令定义（task-init, task-exec, task-adjust 等）
...
- **命令（commands/）**：每个 `.md` 文件定义一个斜杠命令，frontmatter 中声明 `description`、`argument-hint`、`allowed-tools`
- **技能（skills/）**：`SKILL.md` 通过 frontmatter 的 `description` 字段控制自动激活条件（如检测到 `docs/workflow/workflow.json` 时激活）
...
- 修改命令行为时，确保与 `skills/structured-workflow/SKILL.md` 中描述的核心原则保持一致（七项核心原则、六步执行协议等）
