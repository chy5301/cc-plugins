...
### 实测发现：Graph 读回时把日期归一化为 UTC

Task 0 探针用同一个时刻、两种时区写法各建了一个任务：
...
换算本身是正确的（`+08:00` 的 `2026-12-31 09:00` 确实等于 UTC 的 `2026-12-30 16:00`），但**写入时提交的时区不会被原样保留**——Graph 把 `dateTimeTimeZone` 读回时统一归一化为 `UTC`。

后果：

- Agent 写完任务再读回确认时，会看到一个**字面上与提交值完全不同**的 `dateTime`/`timeZone`。这**不是写入失败**，不要据此认为提交没有生效，更不要去"修正"这个值。
- **任何"读回比对"的逻辑都必须先把提交值与回显值换算到同一时区（或同一时刻的绝对值）再比较，不能做字符串比较**——直接比较 `"2026-12-31T09:00:00"` 和 `"2026-12-30T16:00:00.0000000"` 这两个字符串永远不相等，即使它们代表同一时刻。
