...
本 skill 调用的子命令里，明确"可重试"的只有三种情况：退出码 `6`（`AUTH_PENDING`，属于 `setup-guide` 的登录流程）、退出码 `5` 且 `metadata.retry_after_seconds` 给出了等待时长、退出码 `1` 且错误码为 `AUTH_REFRESH_FAILED` 或 `NETWORK_ERROR`（暂时性故障，稍等片刻重试一次即可）。**其余非 0 退出码（如 `2` 的 `INVALID_PARAMETER`、`3` 的"未找到"、`4` 的 `AUTH_EXPIRED`，以及内置清单触发的 `update-list`/`delete-list` 失败）都是终态失败**，遇到时先停下来读 `error.suggestion`，而不是换个参数再试一次。
