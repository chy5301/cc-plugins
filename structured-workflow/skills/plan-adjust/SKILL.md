...
> **注意**：初始规划已合并到 `/workflow-init` 命令中。本命令仅用于对已有计划进行增量变更。
...
3. 判断：
   - TASK_PLAN.md **不存在** → 提示用户：`TASK_PLAN.md 不存在，请先运行 /workflow-init 完成初始化和规划。`，然后终止
   - TASK_PLAN.md **已存在** 且 `$ARGUMENTS` 为空 → 提示用户：`请提供变更描述。用法：/plan-adjust <变更描述>`，然后终止
