...
6. **等待用户确认**后再继续
7. 如需修改计划，建议用户使用 `/task-adjust [变更描述]`
...
| **新增任务** | 发现缺少前置/后续工作 | `/task-adjust 在<PREFIX>-XX后面加一个XX任务` |
| **删除任务** | 已被其他任务覆盖或不再需要 | `/task-adjust 删除<PREFIX>-XX` |
| **修改任务** | 前序任务改变了后续任务的前提 | `/task-adjust <PREFIX>-XX方案改为XX` |
| **重排序** | 依赖关系变化 | `/task-adjust 调整<PREFIX>-XX和<PREFIX>-YY的顺序` |

### 变更规则

1. **执行中的小变更**（仅拆分当前任务）：在 `/task-exec` 中直接处理
2. **计划层面的变更**（新增/删除/修改/重排序）：使用 `/task-adjust [变更描述]`
...
当整个工作流需要被放弃时，使用 `/task-abort` 终止工作流。

### 终止 vs 暂停 vs 归档

| 操作 | 命令 | 适用场景 | 代码影响 |
|------|------|----------|----------|
| **终止** | `/task-abort` | 放弃整个工作流 | 默认不触碰代码；`--reset` 回滚全部 |
| **暂停** | `/task-pause` | 当前任务遇到问题，需要分析 | 不修改代码 |
| **归档** | `/task-archive` | 工作流正常完成 | 不修改代码 |
...
- **仅当前任务有问题**：使用 `/task-pause` 分析问题，或 `/task-adjust 删除` 取消单个任务
- **需要调整计划**：使用 `/task-adjust [变更描述]` 增量调整
