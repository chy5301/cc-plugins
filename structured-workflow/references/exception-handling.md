...
**处理程序**：
1. 停止实施，保存当前进度
2. 将任务拆分为子任务（编号格式：`XX-a`、`XX-b`、`XX-c`）
3. 每个子任务必须遵循 `task-format.md` 格式，并满足粒度原则
4. 更新 TASK_PLAN.md（添加子任务，标记原任务为"已拆分"）
5. 更新 TASK_STATUS.md（记录拆分原因）
6. 仅执行第一个子任务（`XX-a`），后续子任务留给下次会话
7. 在交接记录中说明拆分情况
...
6. **等待用户确认**后再继续
7. 如需修改计划，建议用户使用 `/plan-adjust [变更描述]`
...
| **新增任务** | 发现缺少前置/后续工作 | `/plan-adjust 在XX后面加一个XX任务` |
| **删除任务** | 已被其他任务覆盖或不再需要 | `/plan-adjust 删除XX` |
| **修改任务** | 前序任务改变了后续任务的前提 | `/plan-adjust XX方案改为XX` |
| **重排序** | 依赖关系变化 | `/plan-adjust 调整XX和YY的顺序` |

### 变更规则

1. **执行中的小变更**（仅拆分当前任务）：在 `/task-exec` 中直接处理
2. **计划层面的变更**（新增/删除/修改/重排序）：使用 `/plan-adjust [变更描述]`
3. **阶段性批量调整**：在 `/phase-review` 中系统性评估后输出调整建议
...
当整个工作流需要被放弃时，使用 `/workflow-abort` 终止工作流。

### 终止 vs 暂停 vs 归档

| 操作 | 命令 | 适用场景 | 代码影响 |
|------|------|----------|----------|
| **终止** | `/workflow-abort` | 放弃整个工作流 | 默认不触碰代码；`--reset` 回滚全部 |
| **归档** | `/workflow-archive` | 工作流正常完成 | 不修改代码 |

### 适用场景

- **方向性错误**：分析阶段的技术方案被证明不可行，需要推倒重来
- **需求完全变更**：业务需求发生根本性变化，当前工作流的目标已失效
- **项目取消**：整个项目或功能被取消

### 不适用场景

- **仅当前任务有问题**：使用 `/plan-adjust` 调整计划，或 `/plan-adjust 删除` 取消单个任务
- **需要调整计划**：使用 `/plan-adjust [变更描述]` 增量调整
- **阶段性暂停**：记录交接信息后正常结束会话，下次继续
